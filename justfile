...
initialize:
    ...
    gh repo create Netbeheer-Nederland/im-nbnl-topology \
        --description "Information models for the NBNL Topology Profile." \
        --public \
        --disable-wiki

    git init -b main

    git remote add origin git@github.com:Netbeheer-Nederland/im-nbnl-topology.git
    git fetch
    ...
    gh repo edit Netbeheer-Nederland/im-nbnl-topology --default-branch v0
    git branch -D main
    ...
    #  -H "Accept: application/vnd.github+json" \
    #  -H "X-GitHub-Api-Version: 2022-11-28" \
    #  /repos/Netbeheer-Nederland/im-nbnl-topology/actions/permissions \
    #   -F "enabled=true" -f "allowed_actions=all"

    echo "… OK."
    echo
    echo "A GitHub repository has been created and configured at: https://github.com/Netbeheer-Nederland/im-nbnl-topology"
    echo
...
_post-process-linkml-schema:
    ...
    cp "information_models/im_nbnl_topology.schema.linkml.yml" "artifacts/information_models/"
    @echo
    @echo "Setting version in LinkML schema…"
    @echo
    sed -i '/^version: .*$/d' "artifacts/information_models/im_nbnl_topology.schema.linkml.yml"
    @if [ -z ${VERSION:-} ]; then \
        sed -i "/^name: .*$/a version: {{shell(ref_name)}}" "artifacts/information_models/im_nbnl_topology.schema.linkml.yml"; \
    else \
        sed -i "/^name: .*$/a version: ${VERSION}" "artifacts/information_models/im_nbnl_topology.schema.linkml.yml"; \
    fi
    @echo "… OK."
    @echo
...
[group("schema")]
edit-schema:
    @${VISUAL:-${EDITOR:-nano}} information_models/im_nbnl_topology.schema.linkml.yml

# Show definition of the class identified by the provided CURIE
[group("schema")]
get-definition curie:
    yq '.classes.* | select(.class_uri == "{{curie}}")' information_models/im_nbnl_topology.schema.linkml.yml

# List all classes in the schema
[group("schema")]
list-classes:
    yq '.classes.* | key' information_models/im_nbnl_topology.schema.linkml.yml
...
generate-json-schema: _post-process-linkml-schema
    ...
    poetry run gen-json-schema \
        --not-closed \
        "artifacts/information_models/im_nbnl_topology.schema.linkml.yml" \
        > "artifacts/schemas/json_schema/im_nbnl_topology.json_schema.json"
    @echo "… OK."
    @echo
    @echo "Generated JSON Schema at: artifacts/schemas/json_schema/im_nbnl_topology.json_schema.json"
    @echo
...
validate-example-data: generate-json-schema generate-example-data
    ...
    for example_file in artifacts/examples/*.json; do \
        [ -f "$example_file" ] || continue; \
        poetry run check-jsonschema --schemafile "artifacts/schemas/json_schema/im_nbnl_topology.json_schema.json" $example_file; \
    done
    @echo "… OK."
    @echo
